...
import useBackProfile from "../../hooks/profiles/useBackProfile";
...
export default function BackProfile({ handle }) {
    // connected to metamask
    const { walletConnected, walletAddress } = useWalletConnected();

    // react query mut
    const mutation = useBackProfile();
    ...
    const handleFormikSubmit = (values, formikHelpers) => {
        ...
        mutation.mutateAsync({
            ...
            resetForm();
            // invalidate queries
            queryClient.invalidateQueries(["backers", handle])
            router.push(`/profiles/${handle}`)
            toast.success("Succesfully backed profile with MATIC!")
        }).catch((e) => {
            displayErrorAlert(e);
            setSubmitting(false);
            return;
        ...
        mutation.mutateAsync({
            ...
            queryClient.invalidateQueries({
                predicate: query => {
                    ...
                    return key0.startsWith('backers')
                }
            })
            router.push(`/profiles/${handle}`)
            toast.success(`Succesfully backed profile with ${name}!`)
        }).catch((e) => {
            displayErrorAlert(e);
            handleError(e);
        })
